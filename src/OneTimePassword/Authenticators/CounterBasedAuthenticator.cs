...
namespace OneTimePassword.Authenticators
{
    public class CounterBasedAuthenticator : Authenticator
    {
        ...
        public const uint RFC_MINIMUM_PASSWORD_LENGTH = 6;
        public const uint RFC_MAXIMUM_PASSWORD_LENGTH = 9;

        public override OneTimePassword GeneratePassword(AuthenticatorAccount account)
        {
            if (!(account is CounterBasedAuthenticatorAccount counterAccount)) throw new ArgumentException("Account is not a HOTP account.", nameof(account));
        ...
        {
            if (length < RFC_MINIMUM_PASSWORD_LENGTH || length > RFC_MAXIMUM_PASSWORD_LENGTH) throw new ArgumentOutOfRangeException(nameof(length), "The generated password must be between 6 and 9 characters long.");
